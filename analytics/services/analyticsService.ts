...
export class AnalyticsService {
  ...
  static async getPlatformStats(filters?: AnalyticsFilters): Promise<PlatformStats> {
    try {
      ...
      const averageCompletionTime = allFeedback.length > 0 
        ? allFeedback.reduce((sum, feedback) => sum + feedback.completionTime, 0) / allFeedback.length 
        : 0;
      
      // Calculate active users (last 30 days)
      const thirtyDaysAgo = new Date();
      thirtyDaysAgo.setDate(thirtyDaysAgo.getDate() - 30);
      const activeUsersLast30Days = accounts.filter(account => 
        account.lastLoginAt && account.lastLoginAt >= thirtyDaysAgo
      ).length;
      
      // Calculate new users (last 7 days)
      const sevenDaysAgo = new Date();
      sevenDaysAgo.setDate(sevenDaysAgo.getDate() - 7);
      const newUsersLast7Days = accounts.filter(account => 
        account.createdAt && account.createdAt >= sevenDaysAgo
      ).length;
  ...
  static async getAccountAnalytics(filters?: AnalyticsFilters): Promise<AccountAnalytics[]> {
    try {
      ...
      for (const account of accounts) {
        ...
        const userFeedback = allFeedback.filter(feedback => feedback.userId === account.walletAddress);
        ...
        const completionTime = userFeedback.length > 0 
          ? userFeedback.reduce((sum, feedback) => sum + feedback.completionTime, 0) / userFeedback.length 
          : 0;
        ...
        const daysSinceLastLogin = account.lastLoginAt 
          ? Math.floor((new Date().getTime() - account.lastLoginAt.getTime()) / (1000 * 60 * 60 * 24))
          : 0;
  ...
  static async getDemoAnalytics(filters?: AnalyticsFilters): Promise<DemoAnalytics[]> {
    try {
      ...
      for (const demoStat of demoStats) {
        ...
        // Calculate clap rate
        const clapRate = demoStat.totalCompletions > 0 ? (demoStat.totalClaps / demoStat.totalCompletions) * 100 : 0;
  ...
  static async getUserEngagementMetrics(filters?: AnalyticsFilters): Promise<UserEngagementMetrics> {
    try {
      const accounts = await this.getAllAccounts();
      const allFeedback = await this.getAllFeedback();
      ...
      const activeUsers = accounts.filter(account => 
        account.lastLoginAt && account.lastLoginAt >= sevenDaysAgo
      ).length;
      
      // Calculate new users (last 7 days)
      const newUsers = accounts.filter(account => 
        account.createdAt && account.createdAt >= sevenDaysAgo
      ).length;
      ...
      // Calculate averages
      const averageDemosPerUser = totalUsers > 0 
      ...
      const totalEngagement = accounts.reduce((sum, account) => {
        ...
        return sum + (demosCompleted.length * 20) + (badgesEarned.length * 10) + (userFeedback.length * 5);
      }, 0);
