...
export default function AnalyticsPage() {
  ...
  const filteredFeedbacks = feedbacks.filter(feedback => {
    ...
    if (searchTerm && !feedback.feedback.toLowerCase().includes(searchTerm.toLowerCase())) return false;
    return true;
  });
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-gray-300 mt-2">Comprehensive insights into user feedback and engagement</p>
            </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        {analyticsData && (
          <div className="mb-8">
            <div className="text-center mb-6">
              ...
              <p className="text-gray-300">Real-time insights into user engagement and platform growth</p>
            </div>
              ...
              <div className="bg-gradient-to-r from-blue-500/20 to-cyan-500/20 rounded-xl p-6 border border-blue-400/30 hover:scale-105 transition-transform duration-200">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-xs text-blue-200 mt-1">+{Math.floor(analyticsData.totalUsers * 0.15)} this week</p>
                  </div>
              ...
              <div className="bg-gradient-to-r from-green-500/20 to-emerald-500/20 rounded-xl p-6 border border-green-400/30 hover:scale-105 transition-transform duration-200">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-xs text-green-200 mt-1">{((analyticsData.totalFeedbacks / analyticsData.totalUsers) * 100).toFixed(0)}% response rate</p>
                  </div>
                  <div className="text-4xl">💬</div>
                </div>
              </div>
              ...
              <div className="bg-gradient-to-r from-yellow-500/20 to-orange-500/20 rounded-xl p-6 border border-yellow-400/30 hover:scale-105 transition-transform duration-200">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-3xl font-bold text-white">{analyticsData.averageRating.toFixed(1)}/5</p>
                    <p className="text-xs text-yellow-200 mt-1">{analyticsData.averageRating >= 4 ? 'Excellent' : analyticsData.averageRating >= 3 ? 'Good' : 'Needs Improvement'}</p>
                  </div>
                  <div className="text-4xl">⭐</div>
                </div>
              </div>
              ...
              <div className="bg-gradient-to-r from-purple-500/20 to-pink-500/20 rounded-xl p-6 border border-purple-400/30 hover:scale-105 transition-transform duration-200">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-3xl font-bold text-white">{analyticsData.completionRate.toFixed(1)}%</p>
                  ...
                  <div className="flex items-center space-x-2">
                    <div className="w-32 bg-gray-700 rounded-full h-2">
                      <div 
                        ...
                        style={{ width: `${(analyticsData.userExperience.trustlessWorkKnowledge.beginner / analyticsData.totalUsers) * 100}%` }}
                      ></div>
                    </div>
                    <span className="text-white font-medium">{analyticsData.userExperience.trustlessWorkKnowledge.beginner}</span>
                  </div>
                </div>
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center space-x-2">
                    <div className="w-32 bg-gray-700 rounded-full h-2">
                      <div 
                        ...
                        style={{ width: `${(analyticsData.userExperience.trustlessWorkKnowledge.intermediate / analyticsData.totalUsers) * 100}%` }}
                      ></div>
                    </div>
                    <span className="text-white font-medium">{analyticsData.userExperience.trustlessWorkKnowledge.intermediate}</span>
                  </div>
                </div>
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center space-x-2">
                    <div className="w-32 bg-gray-700 rounded-full h-2">
                      <div 
                        ...
                        style={{ width: `${(analyticsData.userExperience.trustlessWorkKnowledge.advanced / analyticsData.totalUsers) * 100}%` }}
                      ></div>
                    </div>
                    <span className="text-white font-medium">{analyticsData.userExperience.trustlessWorkKnowledge.advanced}</span>
                  </div>
                </div>
              </div>
            </div>
                ...
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center space-x-2">
                    <div className="w-32 bg-gray-700 rounded-full h-2">
                      <div 
                        className="bg-red-500 h-2 rounded-full" 
                        style={{ width: `${(analyticsData.userExperience.stellarKnowledge.beginner / analyticsData.totalUsers) * 100}%` }}
                      ></div>
                    </div>
                    <span className="text-white font-medium">{analyticsData.userExperience.stellarKnowledge.beginner}</span>
                  </div>
                </div>
                <div className="flex items-center justify-between">
                  <span className="text-gray-300">Intermediate</span>
                  <div className="flex items-center space-x-2">
                    <div className="w-32 bg-gray-700 rounded-full h-2">
                      <div 
                        className="bg-yellow-500 h-2 rounded-full" 
                        style={{ width: `${(analyticsData.userExperience.stellarKnowledge.intermediate / analyticsData.totalUsers) * 100}%` }}
                      ></div>
                    </div>
                    <span className="text-white font-medium">{analyticsData.userExperience.stellarKnowledge.intermediate}</span>
                  </div>
                </div>
                <div className="flex items-center justify-between">
                  <span className="text-gray-300">Advanced</span>
                  <div className="flex items-center space-x-2">
                    <div className="w-32 bg-gray-700 rounded-full h-2">
                      <div 
                        className="bg-green-500 h-2 rounded-full" 
                        style={{ width: `${(analyticsData.userExperience.stellarKnowledge.advanced / analyticsData.totalUsers) * 100}%` }}
                      ></div>
                    </div>
                    <span className="text-white font-medium">{analyticsData.userExperience.stellarKnowledge.advanced}</span>
                  </div>
                </div>
              </div>
            </div>
        ...
        <div className="bg-white/5 rounded-xl p-6 border border-white/10 mb-8">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-xl font-semibold text-white">💬 User Feedback ({filteredFeedbacks.length})</h3>
          </div>
              ...
              <div key={feedback.id} className="bg-white/5 rounded-lg p-4 border border-white/10">
                <div className="flex items-start justify-between mb-3">
                  <div className="flex items-center space-x-3">
                    <div className="flex items-center">
                      {[1, 2, 3, 4, 5].map((star) => (
                        <span key={star} className={star <= feedback.rating ? 'text-yellow-400' : 'text-gray-600'}>
                          ⭐
                        </span>
                      ))}
                    </div>
                ...
                <div className="flex items-center justify-between text-xs text-gray-400">
                  <div className="flex items-center space-x-4">
                    ...
                    <span>{feedback.wouldRecommend ? '👍 Recommends' : '👎 Doesn\'t recommend'}</span>
                  </div>
        ...
        <div className="bg-gradient-to-r from-purple-500/10 to-pink-500/10 rounded-xl p-6 border border-purple-400/30">
          <h3 className="text-2xl font-semibold text-white mb-4">🚀 Get Involved with Mini Games Nexus</h3>
          <p className="text-gray-300 mb-6">
            Interested in contributing to our Web3 playground games? Whether you're a developer, designer, or have great ideas, 
