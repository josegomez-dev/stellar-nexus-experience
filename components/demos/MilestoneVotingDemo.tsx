...
export const MilestoneVotingDemo = () => {
    ...
    try {
      ...
      if (updatedMilestone && updatedMilestone.status === 'approved') {
        ...
        updateTransaction(
          txHash,
          'success',
          `"${milestone?.title}" approved by ${stakeholder?.name}! Milestone ready for release.`
        );
        ...
        updateTransaction(
          txHash,
          'success',
          `"${milestone?.title}" approved by ${stakeholder?.name}. More approvals needed.`
        );
  ...
  async function handleReleaseFunds(milestoneId: string) {
    ...
    try {
      ...
      updateTransaction(
        txHash,
        'success',
        `Funds released successfully for "${milestone?.title}"!`
      );
  ...
  const canReleaseMilestone = (milestone: Milestone) => {
    return (
      milestone.status === 'approved' && milestone.approvals.length >= milestone.requiredApprovals
    );
  };
  ...
  const getMilestoneButtonClass = (milestone: Milestone) => {
    const baseClass =
      'px-4 py-2 rounded-lg transition-colors disabled:opacity-50 disabled:cursor-not-allowed mb-2';
        ...
        <div className='relative z-20 opacity-30 pointer-events-none'>
          <div className='text-center mb-8'>
            <div className='flex items-center justify-between mb-4'>
              ...
              <button className='px-4 py-2 bg-red-500/20 hover:bg-red-500/30 border border-red-400/30 rounded-lg text-red-300 hover:text-red-200 transition-colors'>
                🔄 Reset Demo
              </button>
            </div>
          ...
          <div className='text-center py-20'>
            ...
            <p className='text-white/70 text-lg max-w-md mx-auto'>
              The Milestone Voting Demo is currently under development. This feature will allow
              multiple stakeholders to vote on milestone approvals.
            </p>
          </div>
                    ...
                    {milestones.map(milestone => (
                      <div key={milestone.id} className='p-3 bg-white/5 rounded-lg'>
                        ...
                        <p className='text-xs text-blue-300 mt-1'>
                          {milestone.requiredApprovals} approvals required •{' '}
                    ...
                    <div className='flex items-start justify-between mb-4'>
                      ...
                      <div className='text-right'>
                        <button
                          ...
                          disabled={
                            milestone.status !== 'pending' || milestoneLoadingStates[milestone.id]
                          }
                          ...
                          >
                            ...
                            <div className='text-right'>
                              {milestone.status === 'pending' && (
                                <button
                                  onClick={() =>
                                    handleApproveMilestone(milestone.id, stakeholder.id)
                                  }
                              ...
                              {stakeholder.approvedMilestones.includes(milestone.id) && (
                                <div className='text-xs text-green-300'>
                                  Approved{' '}
                                  {stakeholder.approvalTime
                                    ? new Date(stakeholder.approvalTime).toLocaleTimeString()
                                    : ''}
                                </div>
                              )}
                            </div>
          ...
          <div className='mt-8 p-6 bg-brand-500/10 border border-brand-400/30 rounded-lg'>
            ...
            <ul className='text-brand-200 text-sm space-y-2'>
              <li>
                • <strong>Multi-Stakeholder:</strong> 4 different roles must approve milestones
              </li>
              <li>
                • <strong>Consensus Required:</strong> Each milestone needs a specific number of
                approvals
              </li>
              <li>
                • <strong>Progressive Release:</strong> Funds are released per milestone upon
                approval
              </li>
              <li>
                • <strong>Trustless Governance:</strong> Smart contract enforces approval rules
                automatically
              </li>
              <li>
                • <strong>Transparent Process:</strong> All approvals and progress are visible
                on-chain
              </li>
            </ul>
            <p className='text-brand-200 text-sm mt-3'>
              This demonstrates how complex approval workflows can be automated on Stellar, ensuring
              transparency and reducing the need for manual intervention.
            </p>
