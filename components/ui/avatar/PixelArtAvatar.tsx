...
const generateAvatarConfig = (seed: string): AvatarConfig => {
  ...
  return {
    backgroundColor:
      BACKGROUND_COLORS[Math.floor(seededRandom(seedNum, 1) * BACKGROUND_COLORS.length)],
...
export const PixelArtAvatar = ({
  ...
  return (
    <div className={`inline-block ${className}`} dangerouslySetInnerHTML={{ __html: avatarSvg }} />
  );
};
