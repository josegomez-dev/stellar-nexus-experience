...
const ToastItem = ({ toast, onRemove }: ToastItemProps) => {
  ...
  const getToastStyles = () => {
    const baseStyles =
      'relative p-4 rounded-lg shadow-2xl border-2 backdrop-blur-sm transform transition-all duration-300 ease-out';
  ...
  return (
    ...
    >
      ...
      <div className='flex items-start space-x-3'>
        <div className='text-xl flex-shrink-0 mt-1'>{getToastIcon()}</div>

        <div className='flex-1 min-w-0'>
          <h4 className='font-semibold text-sm mb-1 truncate'>{toast.title}</h4>
          <p className='text-xs opacity-90 leading-relaxed'>{toast.message}</p>
          <p className='text-xs opacity-60 mt-2'>{toast.timestamp.toLocaleTimeString()}</p>
        </div>
      ...
      <div className='absolute inset-0 rounded-lg opacity-20 pointer-events-none'>
        <div
          className={`absolute inset-0 rounded-lg bg-gradient-to-r ${
            toast.type === 'success'
              ? 'from-success-400 to-success-300'
              : toast.type === 'error'
                ? 'from-danger-400 to-danger-300'
                : toast.type === 'warning'
                  ? 'from-warning-400 to-warning-300'
