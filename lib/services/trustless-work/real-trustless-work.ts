...
const TRUSTLESS_WORK_API_BASE =
  process.env.NEXT_PUBLIC_TRUSTLESS_WORK_API || 'https://api.trustlesswork.com';
...
const createEscrowTransactionXDR = async (
  payload: RealInitializePayload,
...
export const validateTestnetConnection = (
  walletData: any
  ...
  if (!walletData.network || walletData.network !== 'TESTNET') {
    return {
      isValid: false,
      message:
        'Please switch your wallet to Stellar Testnet. Current network: ' +
...
export const checkAccountFunding = async (
  publicKey: string
  ...
  try {
    ...
    if (!response.ok) {
      if (response.status === 404) {
        return {
          ...
          balance: '0',
          message:
            'Account not found. Please fund your testnet account at https://friendbot.stellar.org to create it.',
        };
      }
    ...
    return {
      ...
      balance: '0',
      message:
        'Unable to check account status. Please ensure your wallet is connected to Stellar Testnet.',
    };
  }
};

// Real transaction submission helper
export const submitRealTransaction = async (
  signedXDR: string,
  ...
  try {
    ...
    if (error instanceof Error) {
      ...
      if (error.message.includes('op_underfunded')) {
        errorMessage =
          'Insufficient XLM balance. Please fund your testnet account at https://friendbot.stellar.org';
